...
(() => {
  ...
  const SOL_ID_RE = /^[1-9A-HJ-NP-Za-km-z]{32,60}$/;
    ...
    return {
      ...
      send(payload) {
        if (typeof payload === "string") {
          queue.push({ text: payload });
          return;
        }
        if (payload && typeof payload === "object") {
          const t = String(payload.text || "").trim();
          if (!t) return;
          queue.push({ text: t, meta: payload.meta });
        }
      },
  ...
  function createAwesomeDanmakuEngine(overlay, initialSettings) {
    ...
    function applySpeed() {
      try {
        ...
        const desired = Math.max(60, Number(settings.speedPxPerSec || 220));
  ...
  function init() {
    ...
    function setupWithSettings(s) {
      ...
      try {
        if (typeof chrome !== "undefined" && chrome.storage?.onChanged) {
          chrome.storage.onChanged.addListener((changes, area) => {
            ...
            try {
              getDanmakuEngine()?.updateSettings?.(currentSettings);
            } catch {
            ...
            try {
              ...
              if (!prevValid && nextValid) {
                const route = parseRouteInfo();
                if (route?.ca) {
                  if (currentSettings.uiMode === "comment") {
      ...
      try {
        ...
        const engine =
          ...
          window.AwesomeDanmaku?.getPlayer
            ...
            : createCanvasDanmakuEngine(overlay, portal.canvas, currentSettings);
          ...
          document.addEventListener(
            ...
            (e) => {
              try {
                ...
                if (el instanceof HTMLElement) {
                  ...
                  if (found?.id) {
                    ...
                    if (frozenAwesomeEl === found.node) {
                      resumeAwesomeNode(found.node, Number(s.speedPxPerSec || 220));
                      frozenAwesomeEl = null;
                      hideTip();
                      return;
                    }
                    if (frozenAwesomeEl) resumeAwesomeNode(frozenAwesomeEl, Number(s.speedPxPerSec || 220));
                ...
                if (frozenAwesomeEl) {
                  const s = getDanmakuEngine()?.getSettings?.() || DEFAULT_SETTINGS;
                  resumeAwesomeNode(frozenAwesomeEl, Number(s.speedPxPerSec || 220));
                  frozenAwesomeEl = null;
                }
      ...
      window.setTimeout(() => {
        const route = parseRouteInfo();
        if (route?.ca && currentSettings.uiMode === "comment" && currentSettings.accessToken) {
          ...
          if (!engine) {
            ...
            window.setTimeout(() => {
              ...
              if (route2?.ca && engine2 && currentSettings.uiMode === "comment" && currentSettings.accessToken) {
  ...
  function bindDock(uiRoot, getSettingsRef, setSettingsRef) {
    ...
    btn.addEventListener("click", () => openInPageConfigModal(getSettingsRef()));
    ...
    async function sendToChat(text) {
      ...
      if (!route) return;
      const ca = route.ca;
      let s = getSettingsRef();
      try {
        s = await ensureAuth(s);
        setSettingsRef(s);
      } catch {
    ...
    async function sendToComment(text) {
      ...
      const u = new URL(s.httpBaseUrl || DEFAULT_HTTP_BASE);
      u.pathname = "/comments";
      const res = await fetch(u.toString(), {
        method: "POST",
        headers: {
          "content-type": "application/json",
          authorization: `Bearer ${String(s.accessToken || "")}`,
        },
        body: JSON.stringify({
          ca,
          text,
      ...
      try {
        ...
        if (cm?.ensurePanel && cm?.open) {
          ...
          if (p instanceof HTMLElement && p.style.display !== "none") {
            await cm.open(ca, String(s.accessToken || ""));
          }
        }
      } catch {
    ...
    if (chatBtn instanceof HTMLButtonElement) {
      ...
      chatBtn.addEventListener("click", () => {
        ...
        if (mode === "comment") {
          const route = parseRouteInfo();
          if (!route?.ca) {
            showToast("无法获取合约地址，请确保在支持的页面（如 pump.fun、GMGN 等）", "error", 3000);
            return;
          }
  ...
  function openInPageConfigModal(settings) {
    if (document.querySelector(".tv-danmaku-modal-backdrop")) return;
    ...
    const body = document.createElement("div");
    const now = Date.now();
    const loggedIn = Boolean(settings.accessToken) && Number(settings.expiresAt || 0) > now + 5_000;
    const addrShort = loggedIn ? shortenAddr(String(settings.address || "")) : "-";
    const expiresAtStr = loggedIn ? new Date(Number(settings.expiresAt)).toLocaleString() : "-";
      ...
      <div class="tv-danmaku-modal-panel is-active" data-panel="cfg">
        ...
        ${loggedIn ? `
          <div style="margin-top: 20px;">
            ...
            <input type="text" id="tvNickname" class="tv-danmaku-modal-input" maxlength="16" placeholder="默认=地址" value="${String(settings.nickname || "").trim() || (settings.address ? defaultNicknameFromAddress(settings.address) : "")}" />
          </div>
        ` : ""}
        <div style="margin-top: 20px;">
          <label style="display: block; margin-bottom: 8px; color: rgba(255,255,255,0.68); font-size: 13px;">HTTP Base</label>
          <input type="text" id="tvHttpBase" class="tv-danmaku-modal-input" placeholder="https://sendit.opsat.io" value="${String(settings.httpBaseUrl || DEFAULT_HTTP_BASE)}" />
        </div>
        <div style="margin-top: 12px;">
          <label style="display: block; margin-bottom: 8px; color: rgba(255,255,255,0.68); font-size: 13px;">WS Base</label>
          <input type="text" id="tvWsBase" class="tv-danmaku-modal-input" placeholder="wss://sendit.opsat.io" value="${String(settings.wsBaseUrl || DEFAULT_WS_BASE)}" />
        </div>
      </div>
      
      ${typeof window !== 'undefined' && window.__tvDanmakuPanel ? window.__tvDanmakuPanel.renderDanmakuPanel(settings, {
    ...
    const httpBaseEl = body.querySelector("#tvHttpBase");
    const wsBaseEl = body.querySelector("#tvWsBase");
    ...
    const gapEl = body.querySelector("#tvGap");
    const gapLabelEl = body.querySelector("#tvGapLabel");
    const colorEl = body.querySelector("#tvColor");
    ...
    async function autoSave() {
      ...
      saveTimer = setTimeout(async () => {
        ...
        const next = {
          ...
          httpBaseUrl: httpBaseEl ? String(httpBaseEl.value || "").trim() || DEFAULT_HTTP_BASE : cur.httpBaseUrl,
          wsBaseUrl: wsBaseEl ? String(wsBaseEl.value || "").trim() || DEFAULT_WS_BASE : cur.wsBaseUrl,
          danmakuArea: areaEl ? String(areaEl.value || "full") : cur.danmakuArea,
          speedPxPerSec: speedEl ? Number(speedEl.value || 220) : cur.speedPxPerSec,
          lanes: gapEl ? Number(gapEl.value || 0) : cur.lanes,
          color: colorEl ? String(colorEl.value || "#ffffff") : cur.color,
        };
        await saveSettings(next);
        tip("已自动保存");
    ...
    httpBaseEl?.addEventListener("change", () => void autoSave());
    wsBaseEl?.addEventListener("change", () => void autoSave());
    ...
    gapEl?.addEventListener("input", () => {
      if (gapLabelEl) gapLabelEl.textContent = String(gapEl.value);
      void autoSave();
    });
    colorEl?.addEventListener("change", () => void autoSave());
    closeBtn.addEventListener("click", close);
    backdrop.addEventListener("click", (e) => {
      if (e.target === backdrop) close();
    });
  ...
  function startCaRoomWatcher(getSettingsRef, setSettingsRef) {
    ...
    ws.on("message", (msg) => {
      ...
      try {
        const s = getSettingsRef();
        const mode = s?.uiMode === "danmaku" ? "danmaku" : "comment";
        if (mode === "danmaku" && msg?.type === "chat_message" && msg?.text) {
          const id = msg?.clientMsgId ? String(msg.clientMsgId) : "";
          if (id) {
            if (seenClientMsgIds.has(id)) return;
    ...
    function tick() {
      ...
      if (caChanged) {
        ...
        const s = getSettingsRef();
        if (s?.uiMode === "comment" && ca && s.accessToken) {
