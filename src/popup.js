...
document.getElementById("pinToSidePanel")?.addEventListener("click", async () => {
  ...
  try {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      sp
        .setOptions({ tabId, enabled: true, path: "src/popup.html" })
        .then(() => sp.open({ tabId }))
        .catch((e) => {
    ...
    if (!loggedIn) {
      ...
      Lobby.mount(root, {
        ...
        onCreate: () => {
          ...
          const tipEl = root.querySelector("#tvVoiceCreateTip");
          if (tipEl) {
            tipEl.textContent = msg;
            tipEl.style.color = "rgba(255, 100, 100, 0.9)";
          }
        },
        onJoin: () => {
          const msg = expiresAt ? "登录已过期：请先在支持页面重新登录。" : "未登录：请先在支持页面完成 OKX 登录。";
          ...
          const tipEl = root.querySelector("#tvVoiceJoinTip");
          if (tipEl) {
            tipEl.textContent = msg;
            tipEl.style.color = "rgba(255, 100, 100, 0.9)";
          }
